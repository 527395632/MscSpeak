        ...
        public override void Play(Stream wavStream)
        {
            ...
            lock (this)
            {
                try
                {
                    ...
                    {
                        var format = new WaveFormat((int)header.SampleRate, header.BitsPerSample, header.NumChannels);
                        if (WinmmNative.WaveOutOpen(out _hwd, -1, ref format, IntPtr.Zero, IntPtr.Zero, 0) == 0)
                        {
